...
model_hparams = {
    ...
    # TODO: 为下面每个模型指定超参数
    ##################################################
    "Decision Tree": dict(criterion='entropy', max_depth=20),
    "Random Forest": dict(criterion='log_loss', n_estimators=200),
    "Bagging": dict(n_estimators=10),
    "Gradient Boosting": dict(n_estimators=200, learning_rate=1e-1),
    "XGBoost": dict(),
    "Naive Bayes": dict(var_smoothing=1e-9),
    "Perceptron": dict(penalty='l1', alpha=1e-4),
    "Logistic Regression": dict(penalty=None, C=0.1, solver='saga'), # define solver to support 'l1' penalty
    "LDA": dict(),
    "SVM": dict(kernel='rbf', C=10)
}
